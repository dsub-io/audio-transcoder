...
describe('resampler WASM runtime', () => {
  it('initializes, caches compiled code, copies PCM, and closes isolated sessions', async () => {
    ...
    expect(exports.dsub_resampler_process).toHaveBeenCalledWith(1, 1, 1, 0);
    ...
    first.close();
    second.close();
    expect(exports.dsub_resampler_destroy).toHaveBeenCalledTimes(2);
  ...
  it('reports a native session creation failure without requiring initialization', async () => {
    ...
    exports.dsub_resampler_last_create_error.mockReturnValue(7);
    stubWebAssembly(exports);
  ...
  it('reports native allocation and processing failures', async () => {
    ...
    exports.dsub_resampler_prepare.mockReturnValueOnce(1);
    expect(() =>
      session.process(new Float32Array(1), new Float32Array(2), false),
    ).toThrow('allocate bounded PCM buffers');
    exports.dsub_resampler_process.mockReturnValueOnce(9);
    expect(() =>
  ...
  it.each([
    ...
    [
      'missing function',
      { ...createExports(), dsub_resampler_process: undefined },
    ],
...
interface FakeExports {
  ...
  readonly dsub_resampler_create: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_destroy: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_input: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_input_frames_used: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_last_create_error: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_output: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_output_frames_gen: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_prepare: ReturnType<typeof vi.fn>;
  readonly dsub_resampler_process: ReturnType<typeof vi.fn>;
}
...
function createExports(
  ...
  return {
    ...
    dsub_resampler_create: vi.fn(() => options.handle ?? 1),
    dsub_resampler_destroy: vi.fn(),
    dsub_resampler_input: vi.fn(() => 0),
    dsub_resampler_input_frames_used: vi.fn(() => options.inputFramesUsed ?? 1),
    dsub_resampler_last_create_error: vi.fn(() => 0),
    dsub_resampler_output: vi.fn(() => 32),
    dsub_resampler_output_frames_gen: vi.fn(
      () => options.outputFramesGenerated ?? 1,
    ),
    dsub_resampler_prepare: vi.fn(() => 0),
    dsub_resampler_process: vi.fn(() => 0),
  };
}
