...
describe('bundled MP3 encoder integration', () => {
  ...
  it('runs the pinned ABI at the supported low-rate edge and rejects invalid native configurations', async () => {
    ...
    const highBitrate = runtime.dsub_mp3_create(2, 48_000, 320_000);
    expect(highBitrate).not.toBe(0);
    runtime.dsub_mp3_destroy(highBitrate);
    expect(runtime.dsub_mp3_create(2, 24_000, 192_000)).toBe(0);
    expect(runtime.dsub_mp3_last_create_error()).toBeLessThan(0);
  });
});
