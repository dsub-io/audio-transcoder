...
describe('Ogg Opus WASM runtime', () => {
  ...
  it('compiles once and gives every encoder session its own bounded memory', async () => {
    ...
    const handle = first.dsub_ogg_opus_create(2, 192_000);
    expect(handle).not.toBe(0);
    expect(first.memory.buffer.byteLength).toBe(8 * 1024 * 1024);
    first.dsub_ogg_opus_destroy(handle);
    expect(first.memory.buffer.byteLength).toBe(8 * 1024 * 1024);
  });
  ...
  it('rejects a module with an incomplete bridge export surface', async () => {
    ...
    Reflect.deleteProperty(exports, 'dsub_ogg_opus_write');
    vi.spyOn(WebAssembly, 'instantiate').mockResolvedValue({
      exports,
    } as unknown as WebAssembly.Instance);
    const { createOggOpusWasmInstantiator } =
    ...
    await expect(instantiateOggOpusWasm()).rejects.toThrow(
      'The Ogg Opus WASM module does not export dsub_ogg_opus_write.',
    );
  });
});
...
function createCompleteExports(): OggOpusWasmExports {
  ...
  return {
    ...
    dsub_ogg_opus_create: zero,
    dsub_ogg_opus_destroy: noResult,
    dsub_ogg_opus_drain: zero,
    dsub_ogg_opus_eos_seen: zero,
    dsub_ogg_opus_last_create_error: zero,
    dsub_ogg_opus_max_page_bytes: zero,
    dsub_ogg_opus_page: zero,
    dsub_ogg_opus_page_length: zero,
    dsub_ogg_opus_pcm: zero,
    dsub_ogg_opus_pcm_capacity_frames: zero,
    dsub_ogg_opus_pull_page: zero,
    dsub_ogg_opus_write: zero,
  };
}
