...
describe('bundled FLAC WASM loading', () => {
  ...
  it('rejects malformed exports and an incompatible ABI', async () => {
    ...
    Reflect.deleteProperty(missingFunction, 'dsub_flac_create');
    mockInstantiation(missingFunction);
    await expect(instantiateBundledFlacWasm(wasmLoader())).rejects.toThrow(
      'does not export dsub_flac_create',
    );

    vi.mocked(WebAssembly.instantiate).mockRestore();
    const incompatible = createFakeWasm();
    vi.spyOn(incompatible, 'dsub_flac_abi_version').mockReturnValue(2);
  ...
  it('surfaces context, PCM, encode, finish, and reset failures', async () => {
    const cases = [
      ...
      ['pcm', (wasm: FakeFlacWasm) => {
        vi.spyOn(wasm, 'dsub_flac_pcm').mockReturnValue(0);
      }, 'invalid PCM buffer'],
  ...
  it.each([
    ...
    async (_name, frameCount, outputLength, frames, message) => {
      ...
      wasm.encode.mockImplementation(() => {
        ...
        vi.spyOn(wasm, 'dsub_flac_frame_count').mockReturnValue(frameCount);
        return 0;
      });
      installFakeRuntime(wasm);
      const Encoder = await loadEncoderConstructor();
...
function createFakeWasm(): FakeFlacWasm {
  ...
  const wasm: FakeFlacWasm = {
    ...
    dsub_flac_abi_version: vi.fn(() => 1),
    dsub_flac_create: create,
    dsub_flac_last_create_error: vi.fn(() => -3),
    dsub_flac_last_error: vi.fn(() => -9),
    dsub_flac_prepare_pcm: preparePcm,
    dsub_flac_pcm: vi.fn(() => 512),
    dsub_flac_encode: encode,
    dsub_flac_output: vi.fn(() => outputPointer),
    dsub_flac_output_length: vi.fn(() => outputLength),
    dsub_flac_frame_count: vi.fn(() => frames.length),
    dsub_flac_frame_size: vi.fn((_handle, index) => frames[index]?.data.byteLength ?? 0),
    dsub_flac_frame_samples: vi.fn((_handle, index) => frames[index]?.samples ?? 0),
    dsub_flac_header: vi.fn(() => headerPointer),
    dsub_flac_header_length: vi.fn(() => headerLength),
    dsub_flac_finish: finish,
    dsub_flac_reset: reset,
    dsub_flac_destroy: destroy,
  };
