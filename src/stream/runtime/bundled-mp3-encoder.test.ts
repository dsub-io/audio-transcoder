...
describe('bundled MP3 WASM loader', () => {
  ...
  it('rejects an ABI mismatch and malformed export surfaces', async () => {
    ...
    vi.mocked(wrongAbi.dsub_mp3_abi_version).mockReturnValue(2);
    instantiate.mockResolvedValueOnce({
      exports: wrongAbi,
    } as unknown as WebAssembly.Instance);
    await expect(
...
describe('bundled MP3 encoder lifecycle', () => {
  it('copies planar PCM into WASM, emits complete frames with metadata once, and resets after flush', async () => {
    ...
    expect(runtime.dsub_mp3_reset).toHaveBeenCalledWith(7);
    ...
    encoder.close();
    encoder.close();
    expect(runtime.dsub_mp3_destroy).toHaveBeenCalledOnce();
  });
  ...
  it('rejects use before init, missing bitrates, and native initialization failures', async () => {
    ...
    vi.mocked(runtime.dsub_mp3_create).mockReturnValue(0);
    vi.mocked(runtime.dsub_mp3_last_create_error).mockReturnValue(-7);
  ...
  it('surfaces native allocation, encode, flush, reset, and incomplete-frame failures', async () => {
    ...
    vi.mocked(runtime.dsub_mp3_prepare_pcm).mockReturnValue(0);
    ...
    vi.mocked(runtime.dsub_mp3_prepare_pcm).mockReturnValue(INPUT_POINTER);
    vi.mocked(runtime.dsub_mp3_encode).mockReturnValue(-12);
    ...
    vi.mocked(runtime.dsub_mp3_encode).mockReturnValue(0);
    vi.mocked(runtime.dsub_mp3_flush).mockReturnValue(-13);
    encoder = configureEncoder(new Encoder(), vi.fn());
    await encoder.init();
    await expect(encoder.flush()).rejects.toThrow('flush failed (-13)');
    encoder.close();

    vi.mocked(runtime.dsub_mp3_flush).mockReturnValue(0);
    vi.mocked(runtime.dsub_mp3_reset).mockReturnValue(-14);
    encoder = configureEncoder(new Encoder(), vi.fn());
    await encoder.init();
    await expect(encoder.flush()).rejects.toThrow('reset failed (-14)');
    encoder.close();

    vi.mocked(runtime.dsub_mp3_reset).mockReturnValue(0);
    vi.mocked(runtime.dsub_mp3_encode).mockImplementation(() => {
  ...
  it('rejects malformed, mismatched, and out-of-memory native output', async () => {
    ...
    vi.mocked(runtime.dsub_mp3_output).mockReturnValue(
    ...
    vi.mocked(runtime.dsub_mp3_output).mockReturnValue(Number.NaN);
    vi.mocked(runtime.dsub_mp3_encode).mockReturnValue(4);
    encoder = configureEncoder(new Encoder(), vi.fn());
    await encoder.init();
    await expect(
      encoder.encode(createSample([Int16Array.of(0)], 0)),
    ...
    vi.mocked(runtime.dsub_mp3_output).mockReturnValue(0);
    vi.mocked(runtime.dsub_mp3_encode).mockReturnValue(Number.NaN);
    encoder = configureEncoder(new Encoder(), vi.fn());
    await encoder.init();
    await expect(
      encoder.encode(createSample([Int16Array.of(0)], 0)),
  ...
  it('rejects a native PCM pointer whose complete planar input is outside memory', async () => {
    const runtime = createFakeRuntime();
    vi.mocked(runtime.dsub_mp3_prepare_pcm).mockReturnValue(
...
function createFakeRuntime(
  ...
  const runtime = {
    ...
    dsub_mp3_abi_version: vi.fn(() => 1),
    dsub_mp3_create: vi.fn(() => 7),
    dsub_mp3_destroy: vi.fn(),
    dsub_mp3_encode: vi.fn(() => {
      new Uint8Array(memory.buffer, OUTPUT_POINTER, encodeBytes.byteLength).set(
        encodeBytes,
      );
      return encodeResult ?? encodeBytes.byteLength;
    }),
    dsub_mp3_flush: vi.fn(() => {
      new Uint8Array(memory.buffer, OUTPUT_POINTER, flushBytes.byteLength).set(
        flushBytes,
      );
      return flushResult ?? flushBytes.byteLength;
    }),
    dsub_mp3_last_create_error: vi.fn(() => 0),
    dsub_mp3_output: vi.fn(() => OUTPUT_POINTER),
    dsub_mp3_prepare_pcm: vi.fn(() => INPUT_POINTER),
    dsub_mp3_reset: vi.fn(() => 0),
    initialize,
