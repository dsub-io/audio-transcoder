...
export const OGG_OPUS_FIXED_SERIAL = 0x4453_5542;
...
function createEncoderFactory(
  ...
  return async (configuration, bitrateBps) => {
    ...
    throwIfAborted(configuration.signal);
    if (wasm.dsub_ogg_opus_max_page_bytes() !== OGG_OPUS_MAX_PAGE_BYTES) {
    ...
    const createdHandle = wasm.dsub_ogg_opus_create(
      configuration.channels,
      bitrateBps,
    );
    if (createdHandle === 0) {
      throw wasmFailure('create', wasm.dsub_ogg_opus_last_create_error());
    }
    ...
    try {
      ...
      wasm.dsub_ogg_opus_destroy(handle);
      throw error;
    }

    const pcmCapacityFrames = wasm.dsub_ogg_opus_pcm_capacity_frames();
    const pcmPointer = wasm.dsub_ogg_opus_pcm(handle);
    ...
    const destroy = (): void => {
      if (handle !== 0) {
        ...
        handle = 0;
        wasm.dsub_ogg_opus_destroy(releasedHandle);
      }
    };
    ...
    const pullAvailablePages = async (): Promise<void> => {
      for (;;) {
        assertRunning();
        const result = wasm.dsub_ogg_opus_pull_page(handle);
        if (result === 0) return;
        if (result < 0) throw wasmFailure('pull an Ogg page', result);
        const pagePointer = wasm.dsub_ogg_opus_page(handle);
        const pageLength = wasm.dsub_ogg_opus_page_length(handle);
        if (
          pagePointer === 0 ||
          pageLength < 27 ||
    ...
    return {
      ...
      finalize(): Promise<void> {
        ...
        finalizeAttempt = (async () => {
          ...
          try {
            throwIfAborted(configuration.signal);
            const result = wasm.dsub_ogg_opus_drain(handle);
            if (result !== 0) throw wasmFailure('drain', result);
            await pullAvailablePages();
            if (wasm.dsub_ogg_opus_eos_seen(handle) !== 1) {
              throw new AudioTranscoderError(
                'WORKER_FAILURE',
      ...
      async write(samples, frameOffset): Promise<void> {
        ...
        try {
          ...
          while (sourceFrame < frameCount) {
            ...
            const result = wasm.dsub_ogg_opus_write(handle, frames);
