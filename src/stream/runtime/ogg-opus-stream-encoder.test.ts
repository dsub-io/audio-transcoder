...
function createFakeWasm(options: FakeWasmOptions = {}): {
  ...
  const wasm = {
    memory,
    dsub_ogg_opus_create: vi.fn(() => options.createHandle ?? 1),
    dsub_ogg_opus_destroy: destroy,
    dsub_ogg_opus_drain: drain,
    dsub_ogg_opus_eos_seen: vi.fn(() => options.eosSeen ?? 1),
    dsub_ogg_opus_last_create_error: vi.fn(() => -17),
    dsub_ogg_opus_max_page_bytes: vi.fn(
      () => options.maxPageBytes ?? OGG_OPUS_MAX_PAGE_BYTES,
    ),
    dsub_ogg_opus_page: vi.fn(() => pagePointer),
    dsub_ogg_opus_page_length: vi.fn(
      () => options.pageLength ?? currentPage.byteLength,
    ),
    dsub_ogg_opus_pcm: vi.fn(() => options.pcmPointer ?? 8),
    dsub_ogg_opus_pcm_capacity_frames: vi.fn(
      () => options.pcmCapacityFrames ?? 960,
    ),
    dsub_ogg_opus_pull_page: vi.fn(() => {
