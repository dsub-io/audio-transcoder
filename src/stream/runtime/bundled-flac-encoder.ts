...
export interface BundledFlacWasmExports extends WebAssembly.Exports {
  ...
  readonly dsub_flac_abi_version: () => number;
  readonly dsub_flac_create: (
    channels: number,
    sampleRate: number,
    bitsPerSample: number,
  ) => number;
  readonly dsub_flac_last_create_error: () => number;
  readonly dsub_flac_last_error: (handle: number) => number;
  readonly dsub_flac_prepare_pcm: (handle: number, frames: number) => number;
  readonly dsub_flac_pcm: (handle: number) => number;
  readonly dsub_flac_encode: (handle: number, frames: number) => number;
  readonly dsub_flac_output: (handle: number) => number;
  readonly dsub_flac_output_length: (handle: number) => number;
  readonly dsub_flac_frame_count: (handle: number) => number;
  readonly dsub_flac_frame_size: (handle: number, index: number) => number;
  readonly dsub_flac_frame_samples: (handle: number, index: number) => number;
  readonly dsub_flac_header: (handle: number) => number;
  readonly dsub_flac_header_length: (handle: number) => number;
  readonly dsub_flac_finish: (handle: number) => number;
  readonly dsub_flac_reset: (handle: number) => number;
  readonly dsub_flac_destroy: (handle: number) => void;
}
...
export async function instantiateBundledFlacWasm(
  ...
  exports._initialize?.();
  const abiVersion = exports.dsub_flac_abi_version();
...
function assertWasmExports(
  ...
  const functions = [
    'dsub_flac_abi_version',
    'dsub_flac_create',
    'dsub_flac_last_create_error',
    'dsub_flac_last_error',
    'dsub_flac_prepare_pcm',
    'dsub_flac_pcm',
    'dsub_flac_encode',
    'dsub_flac_output',
    'dsub_flac_output_length',
    'dsub_flac_frame_count',
    'dsub_flac_frame_size',
    'dsub_flac_frame_samples',
    'dsub_flac_header',
    'dsub_flac_header_length',
    'dsub_flac_finish',
    'dsub_flac_reset',
    'dsub_flac_destroy',
  ] as const;
...
function createBundledFlacEncoder(): typeof CustomAudioEncoder {
  return class BundledFlacEncoder extends CustomAudioEncoder {
    ...
    override async encode(audioSample: AudioSample): Promise<void> {
      ...
      const prepared = wasm.dsub_flac_prepare_pcm(this.#handle, frames);
      if (prepared !== 0) {
        throw nativeFailure('prepare PCM', prepared, wasm, this.#handle);
      }
      const pcmPointer = wasm.dsub_flac_pcm(this.#handle);
      ...
      const encoded = wasm.dsub_flac_encode(this.#handle, frames);
      if (encoded !== 0) {
    ...
    override async flush(): Promise<void> {
      ...
      const finished = wasm.dsub_flac_finish(this.#handle);
      if (finished !== 0) {
        throw nativeFailure('finish', finished, wasm, this.#handle);
      }
      const reset = wasm.dsub_flac_reset(this.#handle);
      if (reset !== 0) {
    ...
    override close(): void {
      if (this.#wasm !== undefined && this.#handle !== 0) {
        this.#wasm.dsub_flac_destroy(this.#handle);
      }
      this.#wasm = undefined;
      this.#handle = 0;
    ...
    #createContext(
      ...
      const handle = wasm.dsub_flac_create(
        this.config.numberOfChannels,
        this.config.sampleRate,
        bitsPerSample,
      );
      if (handle === 0) {
        const error = wasm.dsub_flac_last_create_error();
        throw new Error(
    ...
    #emitPackets(wasm: BundledFlacWasmExports): void {
      const frameCount = wasm.dsub_flac_frame_count(this.#handle);
      const outputLength = wasm.dsub_flac_output_length(this.#handle);
      ...
      const outputPointer = wasm.dsub_flac_output(this.#handle);
      ...
      for (let index = 0; index < frameCount; index += 1) {
        const size = wasm.dsub_flac_frame_size(this.#handle, index);
        const samples = wasm.dsub_flac_frame_samples(this.#handle, index);
...
function readHeader(
  ...
  const pointer = wasm.dsub_flac_header(handle);
  const length = wasm.dsub_flac_header_length(handle);
  if (
    length < 8 ||
...
function nativeFailure(
  ...
  const detail = wasm.dsub_flac_last_error(handle);
  return new Error(
