...
const MP3_HIGH_BITRATE_SAMPLE_RATES = Object.freeze([
  32_000,
  44_100,
  48_000,
]);
...
export interface BundledMp3WasmExports extends WebAssembly.Exports {
  ...
  readonly dsub_mp3_abi_version: () => number;
  readonly dsub_mp3_create: (
    channels: number,
    sampleRate: number,
    bitrate: number,
  ) => number;
  readonly dsub_mp3_destroy: (handle: number) => void;
  readonly dsub_mp3_encode: (handle: number, frames: number) => number;
  readonly dsub_mp3_flush: (handle: number) => number;
  readonly dsub_mp3_last_create_error: () => number;
  readonly dsub_mp3_output: (handle: number) => number;
  readonly dsub_mp3_prepare_pcm: (handle: number, frames: number) => number;
  readonly dsub_mp3_reset: (handle: number) => number;
}
...
export async function instantiateBundledMp3Wasm(
  ...
  exports._initialize?.();
  const actualAbiVersion = exports.dsub_mp3_abi_version();
...
/**
 * Registers dsub's raw-WASM LAME encoder with MediaBunny. The caller owns
...
export function createBundledMp3EncoderRegistration(
  ...
  if (typeof loadWasm !== 'function') {
    throw new TypeError('MP3 runtime-asset registration requires a WASM byte loader.');
  }
  ...
  return class BundledMp3Encoder extends CustomAudioEncoder {
    ...
    override async init(): Promise<void> {
      ...
      if (bitrate === undefined) {
        throw new Error('A bitrate is required for the MP3 runtime-asset encoder.');
      }
      const exports = await instantiateBundledMp3Wasm(
        binding.loadWasm,
        binding.signal,
      );
      const handle = exports.dsub_mp3_create(
      ...
      if (handle === 0) {
        throw new Error(
          `Failed to initialize the MP3 runtime-asset encoder (${exports.dsub_mp3_last_create_error()}).`,
        );
      }
    ...
    override async encode(audioSample: AudioSample): Promise<void> {
      ...
      const inputPointer = exports.dsub_mp3_prepare_pcm(this.#handle, frames);
      if (inputPointer === 0) {
        throw new Error('Failed to allocate the MP3 runtime-asset input buffer.');
      }
      ...
      const encodedBytes = exports.dsub_mp3_encode(this.#handle, frames);
      if (encodedBytes < 0) {
    ...
    override async flush(): Promise<void> {
      const exports = this.#requireExports();
      const flushedBytes = exports.dsub_mp3_flush(this.#handle);
      if (flushedBytes < 0) {
      ...
      const resetResult = exports.dsub_mp3_reset(this.#handle);
      if (resetResult < 0) {
    ...
    override close(): void {
      if (this.#exports !== undefined && this.#handle !== 0) {
        this.#exports.dsub_mp3_destroy(this.#handle);
      }
    ...
    #digestNativeOutput(size: number): void {
      ...
      const exports = this.#requireExports();
      const outputPointer = exports.dsub_mp3_output(this.#handle);
...
export function parseBundledMp3FrameHeader(
  ...
  const totalSize =
    Math.floor(((mpeg1 ? 144 : 72) * bitrate * 1000) / sampleRate) +
    padding;
...
function createImports(): WebAssembly.Imports {
  const unsupportedWasiCall = (): never => {
    throw new Error('The MP3 runtime-asset WASM module attempted unsupported I/O.');
  };
  return {
    env: {
...
function assertExports(
  ...
  if (!(exports.memory instanceof WebAssembly.Memory)) {
    throw new Error('The MP3 runtime-asset WASM module does not export memory.');
  }
  const functions = [
    'dsub_mp3_abi_version',
    'dsub_mp3_create',
    'dsub_mp3_destroy',
    'dsub_mp3_encode',
    'dsub_mp3_flush',
    'dsub_mp3_last_create_error',
    'dsub_mp3_output',
    'dsub_mp3_prepare_pcm',
    'dsub_mp3_reset',
  ] as const;
  for (const name of functions) {
    if (typeof exports[name] !== 'function') {
      throw new Error(`The MP3 runtime-asset WASM module does not export ${name}.`);
    }
  }
}
