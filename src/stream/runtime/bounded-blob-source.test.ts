...
describe('bounded MediaBunny Blob source', () => {
  ...
  it('reads a serializable HTTP source with an exact byte range', async () => {
    ...
    expect(url).toBe('https://www.dsub.io/api/tools/youtube-audio/source');
  ...
  it('defaults HTTP credentials to same-origin and accepts a full 200 response', async () => {
    ...
    vi.stubGlobal('fetch', fetch);
    const input = {
      http: { size: 2, url: 'https://www.dsub.io/source' },
    } as const;
  ...
  it('rejects an HTTP source that ignores a partial range', async () => {
    ...
    await expect(
      readAudioStreamInputRange(
        { http: { size: 3, url: 'https://www.dsub.io/source' } },
        1,
        3,
      ),
  ...
  it('normalizes HTTP fetch failure without leaking the source URL', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => Promise.reject(new Error('secret'))));

    await expect(
      readAudioStreamInputRange(
        { http: { size: 1, url: 'https://www.dsub.io/private?token=secret' } },
        0,
        1,
      ),
    ).rejects.toEqual(
  ...
  it('maps an aborted HTTP request to the package abort error', async () => {
    ...
    const pending = readAudioStreamInputRange(
      { http: { size: 1, url: 'https://www.dsub.io/source' } },
      0,
      1,
      controller.signal,
    );
  ...
  it.each([
    ...
    ['fractional size', { http: { size: 1.5, url: 'https://www.dsub.io' } }],
    ['negative size', { http: { size: -1, url: 'https://www.dsub.io' } }],
    ['invalid credentials', { http: { credentials: 'sometimes', size: 1, url: 'https://www.dsub.io' } }],
    ['relative URL', { http: { size: 1, url: '/source' } }],
    ['non-HTTP URL', { http: { size: 1, url: 'file:///tmp/source' } }],
    ['embedded credentials', { http: { size: 1, url: 'https://user:pass@www.dsub.io/source' } }],
    ['array headers', { http: { headers: [], size: 1, url: 'https://www.dsub.io' } }],
