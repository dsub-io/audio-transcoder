...
describe('strict streaming format matrix', () => {
  ...
  it.each(MATRIX)(
    ...
    async ({ channels, input, output, sampleRate }) => {
      ...
      expect(inspection).toMatchObject({
        ...
        channels,
        codec:
          output.sampleFormat === 'float' ? 'PCM float' : 'PCM integer',
        container: 'WAV',
        sampleRate,
      });
      expect(decoded.channelData).toHaveLength(channels);
      expect(decoded.channelData.every((channel) => channel.length === expectedFrames)).toBe(true);
      expect(
  ...
  it.each(EXPECTED_OUTPUTS)(
    'writes real RF64 for $id',
    async ({ id }) => {
  ...
  return {
    async create() {
      const channelData = Array.from(
        { length: channels },
        (_value, channel) => createSignal(frames, channel),
      );
  ...
  for (let frame = 0; frame < options.frames; frame += 1) {
    for (let channel = 0; channel < options.channels; channel += 1) {
      ...
      if (options.float && options.bitDepth === 32) {
        ...
        writeInt24(view, sampleOffset, toInteger(sample, 24), options.littleEndian);
      } else {
        view.setInt32(sampleOffset, toInteger(sample, 32), options.littleEndian);
      }
    }
  }
}
...
function createSignal(frames: number, channel: number): Float32Array {
  return Float32Array.from(
    { length: frames },
    (_value, frame) => signalAt(frame, channel),
  );
}
